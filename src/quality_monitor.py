...
class RealTimeQualityMonitor:
    ...
    def __init__(self, neo4j_uri: str, neo4j_auth: Tuple[str, str],
        ...
        """
        self.monitor = QualityMonitor(neo4j_uri, neo4j_auth)
    ...
    def start_monitoring(self):
        ...
        logger.info(f"🔄 启动实时质量监控，检查间隔: {self.check_interval}秒")

        # 在实际应用中，这里应该使用多线程或异步任务
        # 这里只是示例代码
        # import threading
    ...
    def _send_critical_alert_notification(self, alerts: List[QualityAlert]):
        """发送严重告警通知"""
        # 这里可以集成邮件、短信、webhook等通知方式
        logger.critical(f"🚨 严重告警: {len(alerts)} 个严重问题需要处理")
        for alert in alerts:
